...
class TestFormatMessages:
    ...
    async def test_format_media(self, example_user: User, media_photos: Media) -> None:
        ...
        event = Event(
            method=EventMethod.MEDIA_PURCHASE,
            object=EventData(
                broadcaster="example_broadcaster",
                user=example_user,
                media=media_photos,
    ...
    async def test_format_message(self, example_user: User, message_example: Message) -> None:
        ...
        event = Event(
            method=EventMethod.PRIVATE_MESSAGE,
            object=EventData(
                broadcaster="example_broadcaster",
                user=example_user,
    ...
    @pytest.mark.parametrize(
        ...
        [
            (EventMethod.USER_ENTER, "example_user entered the room"),
            (EventMethod.USER_LEAVE, "example_user left the room"),
            (EventMethod.FOLLOW, "example_user followed"),
            (EventMethod.UNFOLLOW, "example_user unfollowed"),
            (EventMethod.FANCLUB_JOIN, "example_user joined the fanclub"),
        ],
    )
    @pytest.mark.asyncio
    async def test_format_user_events(
        self, example_user: User, method: EventMethod, expected_message: str
    ) -> None:
    ...
    @pytest.mark.parametrize(
        ("method", "expected_message"),
        [
            (EventMethod.BROADCAST_START, "Broadcast started"),
            (EventMethod.BROADCAST_STOP, "Broadcast stopped"),
        ],
    )
    @pytest.mark.asyncio
    ...
    async def test_format_room_subject_change(self, example_user: User) -> None:
        ...
        event = Event(
            method=EventMethod.ROOM_SUBJECT_CHANGE,
            object=EventData(
                broadcaster="example_broadcaster",
                user=example_user,
    ...
    async def test_format_message_no_message_data(self, example_user: User) -> None:
        ...
        event = Event(
            method=EventMethod.CHAT_MESSAGE,
